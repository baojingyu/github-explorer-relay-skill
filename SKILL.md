...
description: >
  Deep-dive analysis of GitHub projects using Browser Relay (Zero-API cost).
...
> **Mode**: Browser Relay (无需 Search API Key)

## Workflow

[项目名] → [1. 定位 Repo (Browser Relay)] → [2. 多源采集] → [3. 分析研判] → [4. 结构化输出]

### Phase 1: 定位 Repo

**注意：本阶段禁止调用 `web_search`。必须使用 `browser` 工具模拟用户搜索。**

1.  **定位 GitHub 仓库**：
    -   **操作**：调用 `browser` 访问搜索引擎（推荐 Bing 或 Google）。
    -   **指令示例**：`browser(action='open', targetUrl='https://www.bing.com/search?q= <project_name>', profile='chrome')`
    -   **目标**：从搜索结果页面提取第一个匹配 `github.com/{org}/{repo}` 格式的链接。

2.  **社区情报检索**：
    -   **操作**：继续使用 `browser` 查找社区反馈。
    -   **指令示例**：`browser(action='open', targetUrl='https://www.bing.com/search?q= <project_name> review 评测 坑点 vs', profile='chrome')`
...
| **GitHub Issues** | `github.com/{org}/{repo}/issues?q=sort:comments` | Top 3-5 高热度/高质量 Issue | `browser` (渲染动态列表) |
| **技术博客/文档** | Medium/Dev.to/官方文档 | 架构分析、核心概念 | `web_fetch` |
| **社区讨论** | Reddit/V2EX/X (Twitter) | 真实评价、槽点、竞品对比 | `browser` (搜索定位 + 读取) |
...
* **Search Policy**: **Strictly Prohibit** `web_search` tool. All retrieval operations must be performed via `browser` automation (Browser Relay).
* **Relay Strategy**: Use `browser` to navigate to search engines (`bing.com`, `google.com`) and parse results manually.
...
| 依赖 | 类型 | 用途 |
| --- | --- | --- |
| `browser` | 内置工具 | **核心检索工具**。用于访问搜索引擎（Browser Relay）定位项目、查找社区讨论，以及动态页面渲染。 |
